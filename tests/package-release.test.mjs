...
test('two release builds are byte-identical, complete, and normalized', async t => {
  ...
  assert.match(packagedReadme, /Edge 151\.0\.4129\.72 passed the current 19-scenario popup matrix/);
  assert.match(packagedReadme, /native-frozen smoke[\s\S]*final generated artifact tree must still repeat the browser gates before release/);
  ...
  assert.match(packagedNotes, /final rerun is still required/);
  assert.match(packagedNotes, /redesigned Edge frozen smoke still needs its current rerun/);
  assert.doesNotMatch(packagedReadme, /Edge 151\.0\.4129\.72 verification/);
  assert.match(packagedGuide, /native-frozen smoke was redesigned[\s\S]*pending its current source-tree rerun/);

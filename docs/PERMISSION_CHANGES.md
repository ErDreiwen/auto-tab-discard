...
| `scripting` | Read local protection signals and apply the sleep title/favicon marker. | Top frame by default; subframes are touched only when form/media guards require them, and worker-facing results/watchers are bounded. The browser-side physical `allFrames` start cap remains an explicit open P18 item. |
...
No `webNavigation` permission was added to manufacture a physical frame-probe bound. Chrome documents that permission as producing the install warning “Read your browsing history.” Its `getAllFrames` method is the standard route to the frame IDs accepted by targeted scripting, but adding a warned required permission—or an undisclosed optional-permission flow—was rejected for this release. The retained 1,000-frame test distinguishes bounded worker output from the still-open browser-side start count instead of overstating the protection.
